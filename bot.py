...
async def on_message(message: discord.Message):
    ...
    if message.content.startswith('s!adminHelp'):
        if rolecheck == False:
            ...
            return
        embed = discord.Embed(title='Draft Pools commands')
        embed.add_field(name='s!assignDraftPools [ID] (Competitor role)',value='Assign pools on column B to users in column A in the "Bot Management" tab on the given google spreadsheet. Competitor role is optional and will be added alongside draft pool role.', inline=False)
        embed.add_field(name='s!removeDraftPools',value='Remove all Draft Pool roles from users in this server.', inline=False)
        embed.add_field(name='s!setupDraftChannels [ID]',value='Sends the opening messages on each draft pool channel and creates the discussion threads. Messages should be on column A in the "Bot Management" tab on the given google spreadsheet.', inline=False)
        ...
        embed2.add_field(name='s!assignBattlePools [ID]',value='Assign pools on column B to users in column A in the "Bot Management" tab on the given google spreadsheet. Pool roles are configured to be groups of 8 pools (i.e. Battle Pools 1-8).', inline=False)
        embed2.add_field(name='s!removeBattlePools',value='Remove all Battle Pool roles from users in this server.', inline=False)
        embed3 = discord.Embed(title='User verification commands')
        embed3.add_field(name='s!verifyUsers [ID]',value='Checks if any user in the list in the "Bot Management" tab in the doc is verified in the server or not.', inline=False)
        embed3.add_field(name='s!checkPlayers [ID]',value='Checks if any user in the list in the "Bot Management" tab in the doc is not in the server.', inline=False)
        embed4 = discord.Embed(title='Role management commands')
        embed4.add_field(name='s!addSingleRole [ID] [role name]',value='Adds the specified role to all members in the list.', inline=False)
        embed4.add_field(name='s!addMultipleRoles [ID]',value='Goes through the list in the "Bot Management" tab in the doc, adding roles in column B to players in column A. To add multiple roles to the same player, add their name multiple times, with one role per row.', inline=False)
        embed4.add_field(name='s!removeSingleRole [ID] [role name]',value='Removes the specified role from all members in the list.', inline=False)
        embed4.add_field(name='s!removeMultipleRoles [ID]',value='Goes through the list in the "Bot Management" tab in the doc, removing roles in column B from players in column A. To remove multiple roles from the same player, add their name multiple times, with one role per row.', inline=False)
    ...
    if message.content.startswith("s!removeCompetitorRoles"):
        if rolecheck == False:
            await message.channel.send('You do not have the required permissions to run this command!')
            return
        await message.channel.send('**{}: This command has been updated, and is now "s!removeSingleRole".**'.format(f'{message.author.mention}'))
    ...
    if message.content.startswith("s!removeMultipleRoles"):
        ...
        if len(message_args) == 0:
            await message.channel.send('This command requires 1 argument (i.e. s!removeMultipleRoles [id])')
        else:
