...
async def on_message(message: discord.Message):
    ...
    if message.content.startswith('s!adminHelp'):
        ...
        embed3.add_field(name='s!verifyUsers [ID]',value='Checks if any user in the list in the "Not Verified" tab in the doc is verified in the server or not.', inline=False)
        embed3.add_field(name='s!checkPlayers [ID]',value='Checks if any user in the list in the "Player List" tab in the doc is not in the server.', inline=False)
        embed4 = discord.Embed(title='Role removal commands')
        embed4.add_field(name='s!removeSingleRole [role name]',value='Removes the specified role from all members in the server.', inline=False)
        embed4.add_field(name='s!removeMultipleRoles [ID]',value='Goes through the list in the "Role Removal" tab in the doc, removing roles in column B from players in column A. To remove multiple roles from the same player, add their name multiple times, with one role per row.', inline=False)
